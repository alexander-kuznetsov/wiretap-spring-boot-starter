...
public class WiretapStandardLogFieldsProvider {
    ...
    public void writeTo(JsonGenerator gen, ILoggingEvent event) {
        ...
        if (needCaller) {
            ...
            if (callerData != null && callerData.length > 0) {
                ...
                if (props.isVisible(AppLogField.CALLER_CLASS)) {
                    gen.writeStringProperty(f.getCallerClass(), caller.getClassName());
                }
                if (props.isVisible(AppLogField.CALLER_METHOD)) {
                    gen.writeStringProperty(f.getCallerMethod(), caller.getMethodName());
                }
                if (props.isVisible(AppLogField.CALLER_LINE)) {
                    gen.writeNumberProperty(f.getCallerLine(), caller.getLineNumber());
                }
                if (props.isVisible(AppLogField.CALLER_FILE)) {
                    gen.writeStringProperty(f.getCallerFile(), caller.getFileName());
                }
            }
        }
        ...
        if (props.isVisible(AppLogField.MESSAGE)) {
            ...
            gen.writeStringProperty(f.getMessage(), msg);
        }
    ...
    private static void writeJsonMdcField(JsonGenerator gen, String fieldName,
                                          ILoggingEvent event, String mdcKey) {
