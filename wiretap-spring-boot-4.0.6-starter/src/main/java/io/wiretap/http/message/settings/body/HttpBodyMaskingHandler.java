 ...
 * {@link HttpBodyFieldMaskingHandler} (recursive string-level masking) still
 * runs afterwards if it is registered — the two SPIs compose, structural
 * masking is not a replacement.
 *
...
public interface HttpBodyMaskingHandler {
     ...
     *         The first handler returning {@code true} wins per body — subsequent
     *         handlers are not consulted.
     */
