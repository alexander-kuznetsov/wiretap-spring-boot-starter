...
class WebClientLoggingFilterTest {
    ...
    void successfulRequest_capturesHttpInfoIntoMdc() throws Exception {
        ...
        assertThat(logged.get("http_method").asString()).isEqualTo("GET");
        assertThat(logged.get("direction").asString()).isEqualTo("OUTGOING");
        assertThat(logged.get("request_url").asString()).contains("/items/1");
        assertThat(logged.get("response_body").asString()).contains("widget");
    }
    ...
    void postRequest_capturesRequestBody() throws Exception {
        ...
        assertThat(logged.get("request_url").asString()).contains("/orders");
    }
    ...
    void excludedUrl_skipsLogging() {
        ...
        WebClientLoggingFilter filter = new WebClientLoggingFilter(settings, new DefaultBodyParser(null), new HttpAccessFieldNames(), null, null, new io.wiretap.metrics.NoOpWiretapMetrics());
    ...
    void streamingResponse_skipsBodyBufferingAndLogsMarker() throws Exception {
        ...
        JsonNode logged = capturedHttpInfoJson();
        assertThat(logged.get("return_code").asInt()).isEqualTo(200);
        assertThat(logged.get("response_body").asString())
    ...
    void largeResponseBody_truncatesAtMaxBodyLength() throws Exception {
        ...
        JsonNode logged = capturedHttpInfoJson();
        String responseBody = logged.get("response_body").asString();
    ...
    void largeRequestBody_truncatesAtMaxBodyLength() throws Exception {
        ...
        JsonNode logged = capturedHttpInfoJson();
        String requestBody = logged.get("request_body").asString();
    ...
    void wildcardResponseHeaders_logEveryHeaderFromResponse() throws Exception {
        ...
        assertThat(responseHeaders.get("X-Trace").asString()).isEqualTo("abc-123");
    }

    private WebClient clientWith(HttpAccessFieldNames fieldNames) {
        WebClientLogMessageSettings settings = new WebClientLogMessageSettings();
        settings.getHttpBodySettings().setEnableBodyTruncating(false);
        WebClientLoggingFilter filter = new WebClientLoggingFilter(settings, new DefaultBodyParser(null), fieldNames, null, null, new io.wiretap.metrics.NoOpWiretapMetrics());
